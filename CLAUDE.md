...
- **Heartbeat System:** 30-second intervals to measure time-on-page (visibility-aware, pauses when tab hidden)
- **SPA Support:** History API patching (`pushState`/`replaceState` + `popstate`) for efficient route detection
...
- Bounce rate (sessions with 1 page and no heartbeat)
...
- Data characteristics:
  ...
  - **Realistic sessions:** ~40% bounce sessions (single pageview), ~60% engaged sessions (multi-page + heartbeats)
...
### How It Works
1. **Session Management:** Generates cryptographically secure session ID via `crypto.getRandomValues()`, stored in sessionStorage
2. **Initial Tracking:** Sends pageview on load
3. **Heartbeat:** Pings every 30 seconds for time-on-page metrics (pauses when tab is hidden via `visibilitychange`)
4. **SPA Detection:** History API monkey-patching (`pushState`/`replaceState`) + `popstate` listener — fires only on actual navigation, not on every DOM mutation
5. **Reliable Delivery:** Uses `navigator.sendBeacon()` API (non-blocking, survives page unload)

### Minified Version
A minified version (`tracker.min.js`) is auto-generated by the Gradle `minifyTracker` task during build. It strips comments, collapses whitespace, and runs automatically with `processResources`.
...
4. **Analytics:**
   - ✅ Bounce rate calculation (session-based: 1 page + no heartbeat)
   - ✅ Bounce rate stat card with period comparison (inverted colors: lower = good)

5. **Tracker Optimization:**
   - ✅ Tracker rewritten: 1.9KB source, 1.3KB minified (was ~3.2KB)
   - ✅ History API SPA detection (replaced MutationObserver)
   - ✅ Visibility-aware heartbeat (pauses when tab hidden)
   - ✅ Gradle `minifyTracker` task (auto-runs with `processResources`)

6. **Demo & Development:**
   - ✅ Demo data generator (0-3000 realistic events with session simulation)
